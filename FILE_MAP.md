...
`exporter.py`、`manual_import.py` 和 `jianyu_importer.py` 已将主招标结果的
Excel 读写接入 `utils/excel_helper.py`。`bid_results.xlsx` 不存在时仍允许
创建；文件存在但无法读取时会停止，不再将损坏历史视为空表。写回已有
文件前会自动备份，并通过同目录临时文件、回读校验和原子替换保护原文件。

本次迁移不改变 `EXPORT_COLUMNS`、唯一 ID、历史合并、去重、字段映射、
评分、过滤或新增标记规则。手工导入模板和剑鱼 raw 输入的读取也改为读取
失败即报错，但其数据结构与导入规则保持不变。

## V6.6-04 客户池与跟进任务安全读写

`customer_pool.py`、`followup_manager.py` 和 `candidate_contact_importer.py`
已接入 `utils/excel_helper.py`。客户池写入保留已有未知人工字段及原始列顺序，
缺少的标准字段只追加到末尾；客户匹配、新增、更新、来源合并和联系方式导入
规则不变。

`followup_manager.py` 在写入 `customer_pool.xlsx` 和 `followup_tasks.xlsx` 前
分别备份已有文件。两次写入均使用临时文件和回读验证；任一写入失败时，模块
会从本次备份恢复两个旧文件，原先不存在的输出会被删除。若自动恢复本身失败，
异常信息会列出备份路径，供人工复制恢复。

## V6.6-05 企业采购入口状态安全读写

`enterprise_source_manager.py`、`candidate_importer.py`、
`enterprise_validator.py`、`candidate_ranker.py` 和 `source_discovery.py`
已接入 `utils/excel_helper.py`。企业状态表与候选表读取失败时停止处理，写入
已有文件前自动备份，并使用临时文件、回读验证和原子替换。

状态表管理与候选导入会保留未知人工字段及原始列顺序。验证器读取状态表时
记录 SHA-256、文件大小和纳秒修改时间，并在读取后及最终写入前复核；验证
期间文件发生变化时停止写入，保留外部人工修改。URL 验证、登录判断、候选
排序、候选合并和人工确认规则不变。

## V6.6-06 项目监控与其他正式结果安全读写

`target_company_manager.py`、`high_value_filter.py`、`contact_finder.py`、
`project_monitor.py`、`eia_monitor.py` 和 `expansion_monitor.py` 已接入
`utils/excel_helper.py`。

`target_companies.xlsx` 保留已有人工字段和原始列顺序；高价值线索与联系
方式候选属于可重算结果，但覆盖已有文件前仍自动备份。生产、环评和扩产项目
结果读取损坏历史时会停止，不再以空历史覆盖；写入均使用临时文件、回读验证
和原子替换。项目评分、过滤、唯一 ID、历史合并、推荐和采集规则不变。

## V6.6-07 调试数据与文本输出安全化

`debug_raw_results.py`、`eia_debug.py`、`jianyu_search.py` 和
`test_sample_run.py` 的 Excel 输出已接入安全写入。`raw_jianyu_results.xlsx`
作为后续导入源，在覆盖已有文件前自动备份，写入失败时保留旧文件。
`production_raw_debug.xlsx` 已在 V6.6-06 随生产项目模块完成相同保护。

`reporter.py` 新增 UTF-8 安全文本写入：先写同目录临时文件并回读验证，再
备份旧文件和原子替换；失败时自动恢复。`daily_report.txt` 与
`eia_diagnosis.txt` 的生成内容、格式、字段顺序和文件名保持不变。

## V6.6-08 业务数据路径统一登记

`paths.py` 现在以 `Path(__file__).resolve().parent` 计算项目根目录，统一登记
正式结果、业务状态、候选、项目监控、raw/debug、手工导入和 TXT 输出。
所有业务路径均为项目根目录下的绝对 `Path`，不依赖当前工作目录，在 Windows
和 macOS 上使用相同文件名。

剩余业务脚本已从 `paths.py` 导入路径常量。文件未移动、未改名，默认输出
仍位于项目根目录。`test_sample_run.py` 的 `test_output/*` 是隔离测试路径，
不属于正式业务数据登记范围。

## V6.6-09 发布回归

V6.6.0 发布前完成 48 项离线单元测试、54 个项目 Python 文件的无字节码
语法检查，以及 Excel 不存在、损坏、读取失败、写入失败、备份失败、临时
文件失败和跨工作目录启动场景验证。正式 Excel/TXT 在测试前后的 SHA-256
一致；未运行主流程正式模式或任何真实采集。

当前数据层以 `paths.py` 为路径登记入口，以 `utils/excel_helper.py` 为 Excel
安全读写基础。业务脚本已完成 V6.6 计划范围内的接入，字段、列顺序、唯一 ID、
评分、过滤及历史合并业务规则未在本轮改变。
...
- `paths.py`：BidRadar V6.6.0 路径登记；统一管理业务、调试 Excel 和 TXT 文件路径。
...
| `paths.py` | BidRadar V6.6.0 业务数据路径统一登记 | 否 | 无 | 无 | 业务、导入、监控、调试和报告脚本 | 保留 |
